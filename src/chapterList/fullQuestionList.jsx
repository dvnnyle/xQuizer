...
function FullQuestionList() {
  const [questionsData] = useState(allQuestionsData)
  ...
  const handleNext = () => {
    if (currentQuestionIndex < questionsData.length - 1) {
      ...
      const existingData = localStorage.getItem('quiz_fulllist')
      ...
      localStorage.setItem('quiz_fulllist', JSON.stringify({
        score: finalScore,
  ...
  const getPerformanceFeedback = () => {
    ...
    if (percentage >= 80) return { text: 'Great job! 🎉', color: '#8bc34a' }
    if (percentage >= 70) return { text: 'Good work! 👍', color: '#ffc107' }
    if (percentage >= 60) return { text: 'Not bad! 💪', color: '#ff9800' }
        ...
        >
          ...
          <div className="quiz-container">
            <div className="quiz-header">
              <h2>Answer Review - All Questions</h2>
          ...
          <div className="options-list">
            {currentQuestion.options.map((option, index) => (
              ...
              >
                ...
                <span className="option-text">{option}</span>
                {selectedAnswer === index && <span className="you-chose">You chose</span>}
              </button>
            ))}
          </div>
...
export default FullQuestionList
